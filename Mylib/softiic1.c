...
int I2C1_Soft_Start(void)
{
	...
	SDA1_L;     //SCL为高电平，SDA从高电平到低电平则总线启动
	I2C1_Soft_Delay();
	if(SDA1_read) return 0;	//SDA线为高电平则总线出错,退出（开头已给高电平）
	SDA1_L;
	I2C1_Soft_Delay();
	return 1;	
}
...
{
  	if(!I2C1_Soft_Start())return 0;      //I2C1_Soft_Start(）启动总线
